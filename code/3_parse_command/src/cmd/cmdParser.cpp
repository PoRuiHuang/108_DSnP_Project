...
{
   CmdMap::const_iterator pt;
   for(pt=_cmdMap.begin();pt!=_cmdMap.end();pt++){
      pt->second->help();
   }
   // TODO...
}
...
CmdParser::parseCmd(string& option)
{
   ...
   string strCMD;
   size_t space = myStrGetTok(str, strCMD);
   if(getCmd(strCMD)!=0){
      space = str.find_first_not_of(" ",space);
       if(space!=string::npos){
          option = str.substr(space,  str.size()-space );
      }

      return getCmd(strCMD);
   }
   else
      cerr <<  "Illegal command!! (" << strCMD << ")" << endl;    
